...
class nnUNet(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        ## encoder 
        self.encoder_input = DoubleConv(in_channels=in_channels, out_channels=32, first_stride=1)
        self.encoder_block1 = DoubleConv(in_channels=32, out_channels=64, first_stride=2)
        self.encoder_block2 = DoubleConv(in_channels=64, out_channels=128, first_stride=2)
        self.encoder_block3 = DoubleConv(in_channels=128, out_channels=256, first_stride=2)
        self.encoder_block4 = DoubleConv(in_channels=256, out_channels=320, first_stride=2)
        self.encoder_block5 = DoubleConv(in_channels=320, out_channels=320, first_stride=2)
        
        
        ## decoder
        self.upsample1 = nn.ConvTranspose2d(in_channels=320, out_channels=320, kernel_size=2, stride=2)
        self.decoder_block1 = DoubleConv(in_channels=320+320, out_channels=320, first_stride=1)
        
        self.upsample2 = nn.ConvTranspose2d(in_channels=320, out_channels=256, kernel_size=2, stride=2)
        self.decoder_block2 = DoubleConv(in_channels=256+256, out_channels=256, first_stride=1)
        
        self.upsample3 = nn.ConvTranspose2d(in_channels=256, out_channels=128, kernel_size=2, stride=2)
        self.decoder_block3 = DoubleConv(in_channels=128+128, out_channels=128, first_stride=1)
        
        self.upsample4 = nn.ConvTranspose2d(in_channels=128, out_channels=64, kernel_size=2, stride=2)
        self.decoder_block4 = DoubleConv(in_channels=64+64, out_channels=64, first_stride=1)
        
        self.upsample5 = nn.ConvTranspose2d(in_channels=64, out_channels=32, kernel_size=2, stride=2)
        self.decoder_block5 = DoubleConv(in_channels=32+32, out_channels=32, first_stride=1)
        
        
        ## output
       # self.deep_supervision3 = nn.Conv3d(in_channels=256, out_channels=out_channels, kernel_size=1, stride=1)
       # self.deep_supervision2 = nn.Conv3d(in_channels=128, out_channels=out_channels, kernel_size=1, stride=1)
       # self.deep_supervision1 = nn.Conv3d(in_channels=64, out_channels=out_channels, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        ...
        return output #, aux_output_1, aux_output_2, aux_output_3
...
if __name__ == "__main__":
    ...
    image_size = 128
    x = torch.Tensor(1, 3, image_size, image_size)
    x.to(device)
    print("x size: {}".format(x.size()))

    model = nnUNet(in_channels=3, out_channels=4)
