...
from custom_timm.layers import PatchEmbed, Mlp, DropPath, AttentionPoolLatent, RmsNorm, PatchDropout, SwiGLUPacked, \
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # print('vision transformer shapes; ', x.shape)
        B, N, C = x.shape
...
class Block(nn.Module):
    def __init__(
        ...
        self.hopfield = Hopfield(input_size = 1024, patch_size = 16) #2048
        self.drop_path = drop_path

        # self.weight_orig = nn.Parameter(torch.ones((1, 197, 768)))
        # self.weight_mem = nn.Parameter(torch.ones((1, 197, 768)))

    def forward(self, x: torch.Tensor, mem: torch.Tensor) -> torch.Tensor:
        # print('first patch size: ', self.pss)
        x1 = x + self.drop_path1(self.ls1(self.attn(self.norm1(x))))#*0.7
        x2 = x + self.drop_path2(self.ls(self.hopfield(self.norm(x), mem)))#*0.3
        # print('--- hopfield out shapes: ', x1.shape, x2.shape)
        x = x + self.drop_path3(self.ls2(self.mlp(self.norm2(x1 + x2))))
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        #         act_layer=act_layer,
        #         mlp_layer=mlp_layer,
        #     )if (i == depth-3) or (i == depth-2) else
        #     Mem_Block(
        #         dim=embed_dim,
        #         num_heads=num_heads,
        #         mlp_ratio=mlp_ratio,
        ...
        self.memory_image = nn.Parameter(torch.rand((1, 1, 224, 224), requires_grad = True))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # print('==== firstt patch: ', self.init_path)
