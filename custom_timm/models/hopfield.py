...
class Mlp_d(nn.Module):
    ...
    def __init__(
        ...
        self.drop1 = nn.Dropout(0.0)
        self.norm = norm_layer(hidden_features) if norm_layer is not None else nn.Identity()
        self.fc2 = nn.Linear(int(hidden_features*patches), out_features, bias=bias)
        self.drop2 = nn.Dropout(0.0)
...
class Mlp_u(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = nn.Linear(in_features, int(hidden_features*patches), bias=bias)
        self.act = act_layer()
        self.drop1 = nn.Dropout(0.0)
        self.norm = norm_layer(hidden_features) if norm_layer is not None else nn.Identity()
        self.fc2 = nn.Linear(hidden_features, out_features, bias=bias)
    ...
    def forward(self, x):
        ...
        x = x.reshape(batch, self.patches, self.hidden_features) #torch.flatten(x, start_dim=1)
        x = self.fc2(x)
        x = self.drop2(x)
        return x
...
class Hopfield(Module):
    ...
    def __init__(self,
        ...
        :param input_size: depth of the input (state pattern) (the last dimention of the input tensor)
        ...
        self.norm_mem_pattern = nn.LayerNorm(
                normalized_shape=input_size, elementwise_affine=normalize_state_pattern_affine,
                eps=normalize_state_pattern_eps)
        ...
        self.ps = patch_size
        self.sequences = 4
        # self.main_memory = nn.Parameter(torch.rand((3, 197, input_size), requires_grad = True))
        # self.main_memory = nn.Parameter(torch.rand((self.sequences, 3072), requires_grad = True))
        self.main_memory = nn.Parameter(torch.rand((3, 1024), requires_grad = True))
        ...
        self.mlp_q = Normal_mlp(
            in_features=1024,
            hidden_features=1024,
            out_features = 1024,
        ) # all 3072

        self.mlp_v = Normal_mlp(
            in_features=1024,
            hidden_features=1024,
            out_features = 1024,
        )
    ...
    def _associate(self, data: Union[Tensor, Tuple[Tensor, Tensor, Tensor]], mem,
        ...
        # print('==== original input shapes: ', stored_pattern.shape, state_pattern.shape, pattern_projection.shape, mem.shape)
        ...
        if self.norm_stored_pattern is not None:
            stored_pattern = self.norm_stored_pattern(input=stored_pattern.reshape(
                shape=(-1, stored_pattern.shape[1]))).reshape(shape=stored_pattern.shape)
        ...
        if self.norm_state_pattern is not None:
            state_pattern = self.norm_state_pattern(input=state_pattern.reshape(
                shape=(-1, state_pattern.shape[1]))).reshape(shape=state_pattern.shape)
        ...
        if self.norm_pattern_projection is not None:
            pattern_projection = self.norm_pattern_projection(input=pattern_projection.reshape(
                shape=(-1, pattern_projection.shape[1]))).reshape(shape=pattern_projection.shape)      # shape=(-1, state_pattern.shape[1]) for normal network

        memory_pattern = self.norm_mem_pattern(input=self.main_memory.reshape(
            shape=(-1, self.main_memory.shape[1]))).reshape(shape=self.main_memory.shape)

        self.sp = state_pattern
        
        # print('==== original input shapes: ', stored_pattern.shape, state_pattern.shape, pattern_projection.shape)
        batch = pattern_projection.shape[0]
        # if batch > 1:
        #     main_memory = self.main_memory.repeat(batch, 1, 1).permute(1, 0, 2)
        # else:
        #     main_memory = self.main_memory

        # print('============= after repeat main memory: ', memory_pattern.shape)
        memory_q = self.mlp_q(memory_pattern)#.permute(1, 0, 2)
        memory_v = self.mlp_v(memory_pattern)#.permute(1, 0, 2)

        pattern_projection = torch.unsqueeze(pattern_projection, dim = 1)
        pattern_projection = pattern_projection.repeat(1, 3, 1)#.permute(1, 0, 2)
        # if batch == 1:
        #     pattern_projection = torch.unsqueeze(pattern_projection, dim = 0)

        memory_q = torch.unsqueeze(memory_q, dim = 0)
        memory_v = torch.unsqueeze(memory_v, dim = 0)

        # print('==== before association shapes: ', memory_q.shape, memory_v.shape, pattern_projection.shape)
        # Apply Hopfield association and optional activation function.
        return self.association_core(
            query=pattern_projection, key=memory_q, value=memory_v,
    ...
    def forward(self, input: Union[Tensor, Tuple[Tensor, Tensor, Tensor]], mem,
                stored_pattern_padding_mask: Optional[Tensor] = None,
        ...
        input = self.to_dm(input)
        # print('dm shape: ', input.shape)

        association_output = self._maybe_transpose(self._associate(
            data=input, mem = mem, return_raw_associations=False,
        ...
        # print('----- input shapes: ', self.ins, self.sp.shape)
        # print('association out shape: ',association_output.shape)
        association_output = self.from_dm(association_output.squeeze(dim = 0))
...
class HopfieldCore(Module):
    ...
    def forward(self,
        ...
        if self.query_as_static and self.key_as_static:
            ...
            # print('query/embed dim shape: ', query.shape, self.embed_dim)
...
def hopfield_core_forward(query,                           # type: Tensor
    ...
    #### Checking update steps and converting it into tensor ####
    ...
    # print('--- update heads: ', update_active_heads.shape) 
    ####################################################################################################################
    ...
    while update_active_heads.any():
        ...
        if update_step == 0:
            if not use_separate_proj_weight: # Q, K and V projections
            ...
            active_xi = xi.masked_select(mask=update_active_heads).view(size=(-1, *xi.shape[1:])) # update active head shape is -> [batch, 1, 1]
        ...
        attn_output_weights = torch.bmm(q, k.transpose(1, 2))   ############################## matrix multiplication ################
    ...
    # print('before attn shape: ', attn_output_weights.shape, v.shape)
    attn_output = torch.bmm(attn_output_weights, v)
    # print('after att shape; ', attn_output.shape)
